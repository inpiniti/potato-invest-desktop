...
# 함수 또는 컴포넌트들이 순수 함수로 작성이 되어 있는지 확인 프롬프트
```
함수 또는 컴포넌트 또는 action 등이 순수 함수로 작성이 되어 있는지 확인해줘
input 과 output이 있는 모든거에 대해서 확인 부탁해.
컴포넌트: /src/components/GEMINI.md
훅: /src/hooks/GEMINI.md
라이브러리: /src/lib/GEMINI.md
저장소: /src/stores/GEMINI.md
...
# 새로운 ui 추가 프롬프트 예시
```
상단의 윈도우 자체 상단바가 표시되고 있다.
현재의 ui와 약간 이질감이 느껴진다.
vscode등의 프로그램은 자체 바를 표시하는거 같아보인다.

vscode 와 비슷하게 자체바가 있으면 좋겠다.
...
오른쪽영역에 _ ㅁ x 아이콘이 있으면 되고
그 옆에는 사이드섹션토글, 하단섹션토글, 우측섹션토클 버튼이 위치하면 좋겠다.

왼쪽영역에 아이콘 및 프로젝트명이 표시되면 된다. (아이콘은 임시로 아무거나 표시되면 된다.)

사이드섹션은 s&p500 등 리스트를 표시하는 영역을 말함
하단섹션은 트레이딩 섹션을 말함
우측섹션은 뉴스 및 커뮤니티 섹션을 말함
```
...
- **Antigravity** - Google Deepmind의 AI 코드 에디터
  ...
  - AI 기반 코드 작성 및 리팩토링
  - 실시간 코드 분석 및 제안
- **Storybook** - UI 컴포넌트 개발 및 테스트 도구
  - 독립적인 환경에서 컴포넌트 시각화 및 검증

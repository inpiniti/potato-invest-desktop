...
- **Storybook**: 컴포넌트 개발 시 `npm run storybook`을 통해 독립적으로 UI를 확인하고 검증하는 것을 권장합니다. 관련 스토리는 `*.stories.tsx` 파일로 작성합니다.
...
- **설명**
  ...
  - **v0.0.45**: 추세 조회 방식을 KHMS 기반 의존성에서 **인터벌 기반**(종목별 1분 타이머)으로 변경하여 무한 트리거 버그 수정
- **예제**
  ```tsx
  <MainContent />
  ```
